...
// 解析虚拟路径
function resolveVirtualPath(path: string) {
    let pathname = window.location.pathname;
    return pathname.substring(0, pathname.indexOf(path)) || "";
}

// 过滤筛选条件
function filterConditions(conditions: any, isFuzzy: boolean = true): any {
...
// 过滤更新数据
function filterUpdate(update: any, hasId: boolean = true): any {
    if (!update) return {};
...
export const Utils = {
    ...
    resolveVirtualPath,		// 解析虚拟路径
    filterConditions,       // 过滤筛选条件
    filterUpdate,           // 过滤更新数据
};
